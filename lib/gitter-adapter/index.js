...
const GitterRoom = require('./gitter-room.js');
...
Adapter.prototype.queueMessage = function(target, message) {
    this.sendPromiseChain = this.sendPromiseChain.then(() => this.sendMessage(target, message));
};
...
Adapter.prototype.joinRoom = function(room) {
  ...
  room.users()
    .then((users) => {
      ...
      c.send(c.host, irc.reply.nameReply, c.nick, channelType, channel, ':' + usernames.join(' '));
...
Adapter.prototype.listUsers = function(channel, key) {
  ...
  const getRoomMembers = cache.get(uri, async () => {
    const room = await this.gitterClient.rooms.join(uri);
    const members = room.users();
    return members;
  });

  getRoomMembers.then((users) => {
    (users || []).forEach((user) => {
...
Adapter.prototype.autoJoin = function() {
  ...
  .then((rooms) => rooms.map((room) => new GitterRoom(room, gc.client, gc.faye))
    .filter((room) => !room.oneToOne && room.lastAccessTime)
    // sort descending (latest touched first)
    .sort((a, b) => (Date.parse(a.lastAccessTime) < Date.parse(b.lastAccessTime) ? 1 : -1))
